...
public:
  CParticleGenInfo(const SObjectTag& part, int frameCount, CSegId segment, const CVector3f& scale,
  ...
  CSegId GetSegmentId() const { return mSegment; }
  ...
  void OffsetTime(float dt) { mCurrentTime += dt; }
  void MarkFinishTime() { mFinishTime = mCurrentTime; }
...
CHECK_SIZEOF(CParticleGenInfo, 0x78)
