...
const uint CMidiManager::CMidiWrapper::GetAudioSysHandle() const { return x0_sysHandle; }
...
void CMidiManager::CMidiWrapper::SetAudioSysHandle(const uint handle) { x0_sysHandle = handle; }
...
void CMidiManager::Stop(const CSfxHandle& handle, ushort fadeTime) {
  ...
  uint sysHandle = mMidiWrappers[handle.GetIndex()].GetAudioSysHandle();
  if (fadeTime == 0) {
...
: x0_songId(-1), x2_groupId(-1), x4_agscId(-1) {
  in.ReadInt32();
  x0_songId = in.ReadInt32();
  x2_groupId = in.ReadInt32();
  x4_agscId = in.ReadInt32();
