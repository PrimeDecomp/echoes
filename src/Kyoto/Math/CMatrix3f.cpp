...
CMatrix3f::CMatrix3f(const CMatrix3f& other, float scale) {
  m00 = other.m00 * scale;
  m01 = other.m01 * scale;
  m02 = other.m02 * scale;
  m10 = other.m10 * scale;
...
CMatrix3f CMatrix3f::FromColumns(const CVector3f& col0, const CVector3f& col1,
                                 const CVector3f& col2) {
  return CMatrix3f(col0.GetX(), col1.GetX(), col2.GetX(), col0.GetY(), col1.GetY(), col2.GetY(),
                   col0.GetZ(), col1.GetZ(), col2.GetZ());
}

CMatrix3f CMatrix3f::RotateX(const CRelAngle& angle) {
  const float dVar1 = sin(angle.AsRadians());
  const float dVar2 = cos(angle.AsRadians());
  return CMatrix3f(1.f, 0.f, 0.f, 0.f, dVar2, -dVar1, 0.f, dVar1, dVar2);
}
...
CMatrix3f CMatrix3f::Orthonormalized() const {
  CVector3f col0(m00, m10, m20);
  CVector3f col1(m01, m11, m21);
  CVector3f up = col1.AsNormalized();
  CVector3f forward = CVector3f::Cross(col0, up).AsNormalized();
  CVector3f right = CVector3f::Cross(up, forward);
  return FromColumns(right, up, forward);
}
