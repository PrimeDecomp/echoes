...
static const CUnitVector3f& XAxis = CVector3f::Right();
static const CUnitVector3f& YAxis = CVector3f::Forward();
static const CUnitVector3f& ZAxis = CVector3f::Up();
...
CQuaternion CQuaternion::FromMatrixColumns(const CVector3f& col0, const CVector3f& col1,
                                           const CVector3f& col2) {
  const float trace = 1.f + col0.GetX() + col1.GetY() + col2.GetZ();
  if (trace > 1.f) {
    const float qw = CMath::SqrtF(trace) / 2.f;
    const float scale = 0.25f / qw;
    const float qx = scale * (col1.GetZ() - col2.GetY());
    const float qy = scale * (col2.GetX() - col0.GetZ());
    const float qz = scale * (col0.GetY() - col1.GetX());
    return CQuaternion(qw, qx, qy, qz);
  }

  int axis;
  if (col1.GetY() > col0.GetX()) {
    axis = col2.GetZ() > col1.GetY() ? 2 : 1;
  } else {
    axis = col2.GetZ() > col0.GetX() ? 2 : 0;
  }

  if (axis == 0) {
    const float size = 2.f * CMath::SqrtF(col0.GetX() - col1.GetY() - col2.GetZ() + 1.f);
    const float scale = 1.f / size;
    const float qw = scale * (col1.GetZ() - col2.GetY());
    const float qx = size / 4.f;
    const float qy = scale * (col1.GetX() + col0.GetY());
    const float qz = scale * (col2.GetX() + col0.GetZ());
    return CQuaternion(qw, qx, qy, qz);
  } else if (axis == 1) {
    const float size = 2.f * CMath::SqrtF(col1.GetY() - col2.GetZ() - col0.GetX() + 1.f);
    const float scale = 1.f / size;
    const float qw = scale * (col2.GetX() - col0.GetZ());
    const float qy = size / 4.f;
    const float qz = scale * (col2.GetY() + col1.GetZ());
    const float qx = scale * (col0.GetY() + col1.GetX());
    return CQuaternion(qw, qx, qy, qz);
  } else {
    const float size = 2.f * CMath::SqrtF(col2.GetZ() - col0.GetX() - col1.GetY() + 1.f);
    const float scale = 1.f / size;
    const float qw = scale * (col0.GetY() - col1.GetX());
    const float qx = scale * (col0.GetZ() + col2.GetX());
    const float qy = scale * (col1.GetZ() + col2.GetY());
    const float qz = size / 4.f;
    return CQuaternion(qw, qx, qy, qz);
  }
}
...
CQuaternion CQuaternion::AxisAngle(const CUnitVector3f& axis, const CRelAngle& angle) {
  const float w = cos(angle.AsRadians() / 2.f);
  return CQuaternion(w, sine(angle / 2.f) * axis);
}
